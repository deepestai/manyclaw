...
## 5. (백스톱) 무한루프 방지 — `botLoopProtection`

봇끼리 **너무 빨리·너무 많이** 주고받으면 자동으로 잠깐 차단하는 "퓨즈". `requireMention`이 못 막는 경우(멘션 달린 핑퐁 등)의 최후 방어선. 규칙 A(2턴 cap)의 하드 백스톱이에요.

> ⚠️ 이 설정의 키/구조는 아직 실기 확인 전 — **jake 머신에서 먼저 켜보고 진짜 억제되는지 테스트한 뒤** 팀에 적용 권장.

```bash
# 지원 여부/스키마 먼저 확인
openclaw config get channels.defaults.botLoopProtection 2>/dev/null || echo "키 확인 필요"
```
예상 형태 (`openclaw.json` 의 `channels.defaults` 밑):
```json
"botLoopProtection": { "maxEventsPerWindow": 10, "windowSeconds": 60, "cooldownSeconds": 300 }
```
- `maxEventsPerWindow`: `windowSeconds`(초) 안에 허용할 봇 이벤트 수 → 넘으면 `cooldownSeconds` 동안 차단
- 값은 테스트 후 조정 (정상 토론은 안 걸리고, 폭주만 걸리게)
...
- [ ] (백스톱) botLoopProtection — jake에서 검증 후 적용
